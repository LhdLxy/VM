...
class Countdown {
	...
	countDown() {
		...
		this.day.html(`${this.formatTime(d)}天`);
		this.hour.html(`${this.formatTime(h)}时`);
		this.minute.html(`${this.formatTime(m)}分`);
		this.second.html(`${this.formatTime(s)}秒`);
	};
	start() {
		this.countDown();
		setInterval(() => {
...
class BlockItem {
	constructor() {
		this.element = $('.block-item');
		this.height = this.getElementHeight();
		this.length = products.length;
		this.multiplier = Math.floor((this.length - 1) / 3) + 1;
	};
	getElementHeight() {
		const marginTop = parseFloat(this.element.css('marginTop'));
		const marginBottom = parseFloat(this.element.css('marginBottom'));
		return this.element.outerHeight() + marginTop + marginBottom;
	};
	setHeight() {
		const height = this.height * this.multiplier + 20;
		$('html').css('--i', `${height}px`);
	};
	init() {
		$(document).ready(() => {
			this.setHeight();
